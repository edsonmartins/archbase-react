  ...
  const buildMenuItemLanguages = () => {
    if (context.languages) {
      return context.languages.map((language) => {
        return <Menu.Item onClick={()=> handleChangeLanguage(language.lang)} icon={getIconByLanguage(language.lang)}>{language.name}</Menu.Item>
      })
    }
  }
    ...
    <Menu shadow="md" width={200} position="bottom-end">
      <Menu.Target>
        <ActionIcon
          ...
          title={i18next.t('toggleLanguage')}
        >
