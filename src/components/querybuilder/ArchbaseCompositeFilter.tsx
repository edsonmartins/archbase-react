								...
								<ArchbaseList
									...
									dataSource={
										new ArchbaseDataSource('dsSortFields', {
											...
											grandTotalRecords:
												this.props.persistenceDelegator.getFilters().length,
											currentPage: 0,
											totalPages: 0,
											pageSize: 999999,
										})
					...
					<div className="filter-apply">
						<Group spacing="xs">
							...
							>
								<Button
									...
									leftIcon={<IconPlus />}
									onClick={(event) =>
										this.props.onActionClick && this.props.onActionClick('new')
									}
								>
									{t('archbase:New')}
								</Button>
							...
							<Tooltip
								withinPortal
								withArrow
								label={`${t('archbase:Remover filtro')}`}
							>
								<Button
									id="btnRemove"
									color="red"
									...
									onClick={(event) =>
										...
										this.props.onActionClick('remove')
									}
								>
									{t('archbase:Remover')}
								</Button>
						...
						<Group spacing="xs">
							...
							<Tooltip
								withinPortal
								withArrow
								label={`${t('archbase:Aplicar filtro')}`}
							>
								<Button
									id="btnApply"
									variant={this.props.variant}
							...
							<Tooltip
								withinPortal
								withArrow
								label={`${t('archbase:Fechar filtro')}`}
							>
								<Button
									id="btnClose"
									variant={this.props.variant}
									...
									onClick={(event) =>
										this.props.onActionClick &&
										this.props.onActionClick('close')
									}
								>
									{`${t('archbase:Close')}`}
								</Button>
					...
					{this.props.activeFilterIndex === NEW_FILTER_INDEX ? (
						<Radio.Group
							...
							onChange={(value: string) =>
								this.onChangeFilterType(value === 'normal' ? 0 : 1)
							}
						>
							<Group mt="xs">
