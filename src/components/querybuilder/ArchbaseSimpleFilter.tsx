...
const rnd = (() => {
	...
	return Object.assign(
		(len: any, ...set: any[]) => [...iter(len, set.flat())].join(''),
		sets,
	);
})();
...
class ArchbaseSimpleFilter extends Component<
	ArchbaseSimpleFilterProps,
	ArchbaseSimpleFilterState
> {
	static defaultProps = {
	...
	UNSAFE_componentWillReceiveProps = (nextProps: ArchbaseSimpleFilterProps) => {
		...
		const currentFilter = nextProps.currentFilter
			...
			: getDefaultEmptyFilter();
		const activeFilterIndex = nextProps.currentFilter
			? nextProps.activeFilterIndex
			: 0;
		const simpleFields = this.createFilterFields(
			nextProps,
			schema,
			currentFilter,
		);
	...
	onChangeSortItem = (field: any, selected: any, order: any, asc_desc: any) => {
		...
		sortFields = sortFields.sort(function (
			a: { order: number },
			b: { order: number },
		) {
	...
	getColor = (color: string) => {
		return this.props.theme!.colors[color][
			this.props.theme!.colorScheme === 'dark' ? 5 : 7
		];
	};
					...
					>
						...
						>
							<div style={{ display: 'flex', alignItems: 'center' }}>
								...
								<Text
									color={
										this.props.theme!.colorScheme === 'dark' ? 'white' : 'black'
									}
								>
									{child.label}
								</Text>
								<SimpleValueSelector
									...
									handleOnChange={(value) =>
										this.onOperatorChanged(rule, value)
									}
									level={0}
								/>
										...
										<ArchbaseList
											...
											dataSource={
												new ArchbaseDataSource('dsSortFields', {
													...
													grandTotalRecords:
														currentFilter.sort.sortFields.length,
													currentPage: 0,
													totalPages: 0,
													pageSize: 999999,
												})
				...
				if (
					...
					return (
						<ArchbaseEdit
							...
							width={'100%'}
							icon={
								searchComponent ? searchComponent : <IconSearch size="1rem" />
							}
