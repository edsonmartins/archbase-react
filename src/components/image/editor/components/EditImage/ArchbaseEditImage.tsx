...
const ArchbaseEditImage = memo(({ image = '', color = '#1e88e5', initialState = _initialState, saveUpdates = () => { } }: ArchbaseEditImageProps) => {
  ...
  return <div className="ArchbaseEditImage">
    <div id="popup" className="popup">
      <div
        ...
        <button className="icon-btn" onClick={() => { onCloseEditPanel(false) }}>
          <span className="material-icons"><IconX/></span>
        </button>
      </div>
      <div className="image-container">
        <div className="image-holder-full">
          {!showCrop && <img id="image-full" src={imageSrc as string} />}
          {showCrop && <ArchbaseCropperWrapper src={imageSrc as string} size={croppSize} croppUpdate={onCroppUpdate} ></ArchbaseCropperWrapper>}
        </div>
        ...
        <div className="control-panel">
          <TabContainer lazy borderLine>
              ...
              {showCrop &&
                <React.Fragment>
                  ...
                  <p style={{ marginBottom: '4px !important' }}>
                      <Button onClick={onCrop} leftIcon={<IconCrop color="orange"/>}>
                        {`${i18next.t('Crop')}`}
                      </Button>
                  </p>
                </React.Fragment>
            ...
            <TabItem disabled={showCrop} name={`${i18next.t('Filters')}`}>
              <BasicFilter color={color} initialState={state.basicFilters} changeFilter={onChangeFilters}></BasicFilter>
            </TabItem>
          </TabContainer>
          <button
          ...
          <div className='flex-row-start' style={{ marginTop: '10px', justifyContent: 'space-between', alignItems: 'center' }}>
            <Button leftIcon={<IconCheck/>} color="green" onClick={() => { onCloseEditPanel(true) }}>{`${i18next.t('archbase:Apply')}`}</Button>
            {sizeImage && <p
