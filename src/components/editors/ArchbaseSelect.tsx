...
import { ArchbaseAsyncSelectContext, ArchbaseAsyncSelectContextValue } from './ArchbaseAsyncSelect.context'
...
export function ArchbaseSelect<T, ID, O>({
	...
	const handleItemComponent = ({option, checked}) => {
		return (
			<ItemComponent {...option} checked={checked}/>
		)
	}

	const renderOption = ItemComponent ? {renderOption: ({option, checked}: ComboboxLikeRenderOptionInput<ComboboxItem>) => handleItemComponent({option, checked})} : {}
