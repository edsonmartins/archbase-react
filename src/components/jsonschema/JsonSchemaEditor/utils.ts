...
export const getDefaultSchema = (dataType: DataType, includeSchema?: boolean): JSONSchema7 => {
	switch (dataType) {
...
export const handleTypeChange = (newValue: JSONSchema7TypeName, rootChange: boolean): JSONSchema7 => {
	switch (newValue) {
		case 'array':
		...
		default:
			return getDefaultSchema((DataType as any)[newValue], rootChange) as JSONSchema7;
	}
};

export const renameKeys = R.curry((keysMap: Record<string, string>, object: any) =>
	R.reduce(
		(acc: any, key: string) => {
