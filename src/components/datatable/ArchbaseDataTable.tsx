...
import { IconDownload, IconPrinter, IconRefresh } from '@tabler/icons'
...
import {filter,isEmpty} from '../core/utils'
...
type ArchbaseDataTableCurrentCell = {
  rowIndex: number
  columnName: string
...
export function ArchbaseDataTable<T extends object, ID>(props: ArchbaseDataTableProps<T, ID>) {
          ...
          if (isValidElement(col) && col.props.visible) {
            let element: any = {
              ...
              accessorFn: (originalRow) =>
                ArchbaseObjectHelper.getNestedProperty(originalRow, col.props.dataField),
              ...
              Cell: col.props.render ? ({ cell }) => col.props.render(cell) : undefined,
              Header: ({ column }) => (
                <i
                  style={{
  ...
  const handlePrint = (table) => {
    ...
    if (table) {
      const collection =
        table.refs.tableContainerRef.current.getElementsByClassName('mantine-Table-root')
...
export interface ArchbaseDataTableColumnProps<T> {
  ...
  minSize?: number
  maxSize?: number
  render?: (cell: MRT_Cell<any>) => ReactNode
  visible: boolean
  size: number
