...
function buildHeader(
	...
	if (options && options.headerGridColumns) {
		const footerGridColumnsLeft: ArchbaseBreakpointsColSpans = options
			.headerGridColumns.left
			? options.headerGridColumns.left
			: { xs: 'auto', sm: 'auto', md: 'auto', lg: 'auto' };
		const footerGridColumnsMiddle: ArchbaseBreakpointsColSpans = options
			.headerGridColumns.middle
			? options.headerGridColumns.middle
			: { xs: 'auto', sm: 'auto', md: 'auto', lg: 'auto' };
		const headerGridColumnsRight: ArchbaseBreakpointsColSpans = options
			.headerGridColumns.right
		...
		const headerFlexGrow =
			options && options.headerFlexGrow ? options.headerFlexGrow : 'even';
		const headerFlexRightJustifyContent =
			options && options.headerFlexRightJustifyContent
				? options.headerFlexRightJustifyContent
				: 'flex-end';
		const headerFlexLeftJustifyContent =
			options && options.headerFlexLeftJustifyContent
				? options.headerFlexLeftJustifyContent
				: 'flex-start';
		...
		return (
			<Flex justify={'space-between'}>
				<Flex
					w={
						headerFlexGrow === 'left' || headerFlexGrow === 'even'
							? '100%'
							: undefined
					}
					maw={
				...
				<Flex
					w={
						headerFlexGrow === 'right' || headerFlexGrow === 'even'
							? '100%'
							: undefined
					}
					maw={
...
function buildFooter(
	...
	if (options && options.footerGridColumns) {
		const footerGridColumnsLeft: ArchbaseBreakpointsColSpans = options
			.footerGridColumns.left
			? options.footerGridColumns.left
			: { xs: 'auto', sm: 'auto', md: 'auto', lg: 'auto' };
		const footerGridColumnsMiddle: ArchbaseBreakpointsColSpans = options
			.footerGridColumns.middle
			? options.footerGridColumns.middle
			: { xs: 'auto', sm: 'auto', md: 'auto', lg: 'auto' };
		const footerGridColumnsRight: ArchbaseBreakpointsColSpans = options
			.footerGridColumns.right
		...
		const footerFlexGrow =
			options && options.footerFlexGrow ? options.footerFlexGrow : 'even';
		const footerFlexRightJustifyContent =
			options && options.footerFlexRightJustifyContent
				? options.footerFlexRightJustifyContent
				: 'flex-end';
		const footerFlexLeftJustifyContent =
			options && options.footerFlexLeftJustifyContent
				? options.footerFlexLeftJustifyContent
				: 'flex-start';
		...
		return (
			<Flex justify={'space-between'}>
				<Flex
					w={
						footerFlexGrow === 'left' || footerFlexGrow === 'even'
							? '100%'
							: undefined
					}
					maw={
				...
				<Flex
					w={
						footerFlexGrow === 'right' || footerFlexGrow === 'even'
							? '100%'
							: undefined
					}
					maw={
...
export function ArchbaseSpaceTemplate<T extends object, ID>({
	...
	useHotkeys([
		[debugOptions && debugOptions.debugLayoutHotKey, () => setDebug(!_debug)],
	]);
		...
		<>
			...
			>
				<ArchbaseSpaceFixed height={'100%'}>
					<ArchbaseSpaceTop size={headerSize.height}>
						<div ref={headerRef}>
							{buildHeader(options, headerLeft, headerRight, _debug)}
						</div>
					</ArchbaseSpaceTop>
					<ArchbaseSpaceFill>
						<ScrollArea
					...
					<ArchbaseSpaceBottom size={footerSize.height}>
						<div ref={footerRef}>
							{buildFooter(options, footerLeft, footerRight, _debug)}
						</div>
					</ArchbaseSpaceBottom>
				</ArchbaseSpaceFixed>
			</Paper>
			<ArchbaseDebugInspector
				debugObjectInspectorHotKey={
					debugOptions && debugOptions.debugObjectInspectorHotKey
				}
