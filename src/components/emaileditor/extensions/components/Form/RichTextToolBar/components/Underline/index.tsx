...
export function Underline(props: LinkProps) {
  ...
  return (
    <Tooltip
      color='#fff'
      position='tl'
      content={t('archbase:Underline')}
    >
      <ToolItem title={t('archbase:Underline')} isActive={Boolean(node)} icon={<ArchbaseEmailIconFont iconName='icon-underline' />} onClick={onClick} />
    </Tooltip>
  );
}
