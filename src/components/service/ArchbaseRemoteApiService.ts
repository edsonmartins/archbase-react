...
import { API_TYPE, IOCContainer } from '@components/core/ioc';
...
export class ArchbaseAxiosRemoteApiClient implements ArchbaseRemoteApiClient {
  ...
  async post<T, R>(url: string, data: T, headers?: Record<string, string>): Promise<R> {
    ...
    const response = await axios.post(url, JSON.stringify(ArchbaseJacksonParser.convertObjectToJson(data)), {
      headers: headersTemp,
    });
...
export abstract class ArchbaseRemoteApiService<T, ID> {
  ...
  async findAllWithSort(page: number, size: number, sort: string[]): Promise<Page<T>> {
    return this.client.get<Page<T>>(`${this.getEndpoint()}/findAll?page=${page}&size=${size}&sort=${sort}`);
  }
  ...
  async findAllWithFilter(filter: string, page: number, size: number): Promise<Page<T>> {
    return this.client.get<Page<T>>(`${this.getEndpoint()}/findWithFilter?page=${page}&size=${size}&filter=${encodeURIComponent(filter)}`);
  }
  ...
  async findAllWithFilterAndSort(filter: string, page: number, size: number, sort: string[]): Promise<Page<T>> {
    return this.client.get<Page<T>>(
      `${this.getEndpoint()}/findWithFilterAndSort?page=${page}&size=${size}&filter=${encodeURIComponent(filter)}&sort=${sort}`,
    );
  }
