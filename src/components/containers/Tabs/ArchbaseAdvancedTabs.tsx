...
const ArchbaseAdvancedTab: React.FC<ArchbaseAdvancedTabProps> = (props) => {
  ...
  return (
    <div
      className={`archbase_tab ${!isAdded ? ' archbase_tab-was-just-added' : ''}${
        startX == null && !activeTab ? ' archbase_tab-was-just-dragged' : ''
      }`}
      style={{
        transform: `translate3d(${movePosition == null ? tmp : movePosition}px, 0, 0)`,
        width: `${width}px`,
      }}
      tab-active={activeTab !== false ? '' : null}
      is-small={contentWidth < TAB_SIZE_SMALL ? '' : null}
      is-smaller={contentWidth < TAB_SIZE_SMALLER ? '' : null}
      is-mini={contentWidth < TAB_SIZE_MINI ? '' : null}
      ref={tabEl}
    >
      ...
      <div className="archbase_tab-background">
        <svg version="1.1" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <symbol id="archbase_tab-geometry-left" viewBox="0 0 214 36">
              <path d="M17 0h197v36H0v-2c4.5 0 9-3.5 9-8V8c0-4.5 3.5-8 8-8z" />
            </symbol>
            <symbol id="archbase_tab-geometry-right" viewBox="0 0 214 36">
              <use xlinkHref="#archbase_tab-geometry-left" />
            </symbol>
            <clipPath id="crop">
          ...
          <svg width="52%" height="100%">
            <use xlinkHref="#archbase_tab-geometry-left" width="214" height="36" className="archbase_tab-geometry" />
          </svg>
          <g transform="scale(-1, 1)">
            <svg width="52%" height="100%" x="-100%" y="0">
              <use xlinkHref="#archbase_tab-geometry-right" width="214" height="36" className="archbase_tab-geometry" />
            </svg>
          </g>
        </svg>
      </div>
      <div className="archbase_tab-content">
        ...
        <div
          className="archbase_tab-drag-handle"
          title={title}
          onClick={onClick}
...
export const ArchbaseAdvancedTabs: React.FC<ArchbaseAdvancedTabsProps> = (props) => {
  ...
  return (
    <div className={className} style={style}>
      <div
        className={`archbase_tabs${!!dark ? ' archbase_tabs-dark-theme' : ''}${
          !isDragging ? ' archbase_tabs-is-sorting' : ''
        }`}
      >
        <div className={`archbase_tabs_content`} ref={tabContentEl}>
          {!!tabs &&
            tabs.map(
              (m: ArchbaseAdvancedTab, index) =>
                !!positions[index] && (
                  <ArchbaseAdvancedTab
                    key={m.key}
                    favicon={m.favicon}
