...
test('Checking window properties', () => {
  ...
  const testWindowTitleBarTitle = testWindowTitleBar?.querySelector('.windowTitle');
  expect(testWindowTitleBarIcon).not.toBeNull();

  const titleBarButtons = testWindowTitleBar?.querySelector<HTMLDivElement>('.buttonContainer');
  expect(titleBarButtons).not.toBeNull();

  const minimizeButton = titleBarButtons?.querySelector<HTMLSpanElement>('.windowButton:nth-child(1)');
  expect(minimizeButton).not.toBeNull();
  expect(minimizeButton?.textContent).toBe('▁');

  const maximizeButton = titleBarButtons?.querySelector<HTMLSpanElement>('.windowButton:nth-child(2)');
