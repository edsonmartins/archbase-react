...
const builder: Builder = {
  comparison(selector, operator, value) {
    return createComparisonNode(
      ...
      operator,
      createValueNode(
        Array.isArray(value) ? value.map((singleValue) => String(singleValue)) : String(value)
      )
    )
  },
  eq(selector, value) {
  ...
  logic(expressions, operator) {
    if (!expressions.length) {
      throw new Error(
        `The logic expression builder requires at least one expression but none passed.`
      )
    }
    ...
    return expressions
      ...
      .reduce(
        (leftExpression, rightExpression) =>
          createLogicNode(leftExpression, operator, rightExpression),
        expressions[0]
      )
  },
  and(...expressions) {
