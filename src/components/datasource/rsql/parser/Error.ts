...
function createErrorForUnexpectedToken(token: AnyToken, source: string): SyntaxError {
  return new SyntaxError(
    isEndToken(token)
      ...
      : `Unexpected ${token.value.length > 1 ? 'string' : 'character'} '${
          token.value
        }' at position ${token.position + 1} in "${source}".`
  )
}
...
function createErrorForUnclosedParenthesis(
  ...
  return new SyntaxError(
    `Unexpected end in "${source}". Did you forget to close parenthesis at position ${
      parentPosition + 1
    }?`
  )
}

function createErrorForEmptyInput(_token: AnyToken, source: string): SyntaxError {
