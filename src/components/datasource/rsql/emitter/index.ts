...
function selectQuote(
  ...
  if (optimizeQuotes) {
    ...
    return countQuote(value, otherQuote) < countQuote(value, preferredQuote)
      ? otherQuote
      : preferredQuote
  }
...
function emitLogic(node: LogicNode, options: EmitOptions) {
  ...
  const operator =
    node.operator === AND_VERBOSE || node.operator === OR_VERBOSE
      ? ` ${node.operator} `
      : node.operator
...
function emitWithoutOptionsValidation(expression: ExpressionNode, options: EmitOptions): string {
  ...
  throw new TypeError(
    `The "expression" has to be a valid "ExpressionNode", ${String(expression)} passed.`
  )
}

function emit(expression: ExpressionNode, options: EmitOptions = {}) {
  if (
    options.preferredQuote !== undefined &&
