...
type ComparisonNode = BinaryNode<
  typeof NodeType.COMPARISON,
  SelectorNode,
  ComparisonOperator,
  ValueNode
>
...
): candidate is ComparisonNode {
  return (
    isNode(candidate) &&
    candidate.type === NodeType.COMPARISON &&
    (operator === undefined ||
      isComparisonOperator((candidate as ComparisonNode).operator, operator))
  )
}
...
function getSelector(comparison: ComparisonNode): string {
  if (!isComparisonNode(comparison)) {
    throw new TypeError(
      `The "comparison" has to be a valid "ComparisonNode", ${String(comparison)} passed.`
    )
  }
...
function createComparisonNode(
  ...
  if (!skipChecks) {
    if (!isSelectorNode(selector)) {
      throw new TypeError(
        `The "selector" has to be a "SelectorNode", "${String(selector)}" passed.`
      )
    }
    if (typeof operator !== 'string') {
      throw new TypeError(
        `The "operator" has to be a "SelectorNode", "${String(operator)}" passed.`
      )
    }
    if (!isComparisonOperator(operator)) {
      throw new TypeError(
        `The "operator" has to be a valid "ComparisonOperator", ${String(operator)} passed.`
      )
    }
...
function createLogicNode(
  ...
  if (!skipChecks) {
    ...
    if (!isLogicOperator(operator)) {
      throw new TypeError(
        `The "operator" has to be a valid "LogicOperator", ${String(operator)} passed.`
      )
    }
