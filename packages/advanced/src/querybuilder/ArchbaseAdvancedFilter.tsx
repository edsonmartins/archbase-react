...
import { ArchbaseDateTimePickerEdit, ArchbaseDateTimePickerRange, ArchbaseSelect, ArchbaseSelectItem, ArchbaseSwitch } from '@archbase/components'
...
> {
  ...
  UNSAFE_componentWillReceiveProps = (nextProps: CustomSortItemProps<T, ID>) => {
    this.setState({
      update: Math.random(),
...
class RuleGroupItem extends Component<RuleGroupItemProps> {
        ...
        <dt className="rules-group-header">
          ...
          >
            <Switch
              ...
              onChange={(event) => this.onConditionChange(event.currentTarget.checked ? 0 : 1)}
  ...
  onNotConditionChange(_value, checked) {
    const { onPropChange } = this.props.schema
    onPropChange('not', _value === true ? 'not' : '', this.props.id)
  }
...
class RuleItem extends Component<RuleItemProps> {
    ...
    return (
      <li className={'rule-container'}>
        <Checkbox
          ...
          checked={!disabled}
          style={{ cursor: 'pointer' }}
          onChange={(event) => this.onDisabledChanged(event.currentTarget.checked)}
        />
        <ValueSelector
          options={fields}
          value={field!}
  ...
  onDisabledChanged = (_value) => {
    this.onElementChanged('disabled', !_value)
  }
