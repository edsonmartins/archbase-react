...
const TabContainer = memo(({
  ...
  useEffect(() => {
    ...
    if (activeIndex > tabs.length - 1 || activeIndex < 0)
      throw new Error(`Intervalo inválido para activeIndex ${activeIndex}`);
    setHeader(tabs);
  }, [children])
  ...
  function checkChildrens(data: any[]) {
    try {
      data.map((el: any) => {
        if (!el.props.name)
          throw new Error('Dentro do componente TabContainer só pode ser renderizado o componente TabItem e precisa de um suporte de nome')
        return true;
      })
    } catch (e) {
      throw new Error("O componente TabContainer deve ter um TabItem como componente filho");
    }
...
export const TabItem = memo(({ name, children, icon = null, type = 'TabItem', disabled = false }:
  {
    name: string;
    children: ReactNode, icon?: React.JSX.Element | string | null | undefined;
    disabled?: boolean
    type?: string
  }) => {
