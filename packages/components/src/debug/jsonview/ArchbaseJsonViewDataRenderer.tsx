...
import { useArchbaseBool } from '@archbase/core';
...
function renderExpandableObject({
	...
	const [expanded, , , toggleExpanded] = useArchbaseBool(shouldInitiallyExpand(level, value, field));

	React.useEffect(() => {
		if (!shouldInitiallyExpandCalledRef.current) {
			shouldInitiallyExpandCalledRef.current = true;
		}
		// Note: useArchbaseBool doesn't provide a direct setter, so we rely on the initial value
	}, [shouldInitiallyExpand]);
